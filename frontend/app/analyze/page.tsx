...
const TEXT_SAMPLES = [
  {
    family: "digital_arrest" as const,
    text: "I am Inspector Sharma from Mumbai Police. If you disconnect, an arrest warrant will be issued today. Do not tell your family or bank. Share your Aadhaar and OTP. Transfer Rs. 50,000 to safe@ybl immediately.",
  },
  {
    family: "kyc_bank_fraud" as const,
    text: "This is an SBI KYC officer. Your account will be blocked tonight. Share the OTP and card number now to complete KYC.",
  },
  {
    family: "legitimate" as const,
    text: "Hello, this is Meera from the SBI MG Road branch confirming your appointment tomorrow. Please visit the branch. We will never ask for an OTP or payment.",
  },
];
...
export default function AnalyzePage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () =>
        setSeconds((value) => {
          if (value + 1 >= MAX_AUDIO_DURATION_SECONDS)
  ...
  async function acceptAudio(candidate: File) {
    ...
    if (!Number.isFinite(duration) || duration > MAX_AUDIO_DURATION_SECONDS + 1)
    ...
    <main className="page-shell analyze-page">
      <div className="page-intro">
        <div>
          <div className="eyebrow">
            <span /> Evidence analyzer
          </div>
          <h1 className="page-title">
            Bring the evidence.
            <br />
            We&apos;ll trace the pressure.
          </h1>
        </div>
        <p>
          Analyze up to three minutes of audio, pasted messages, or a
          screenshot. The report identifies the scam family, maps each playbook
          stage, extracts payment evidence, and prepares the next action.
        </p>
      </div>
      <div className="privacy-ribbon">
        ...
        <div>
          ...
          <small>
            Audio is analyzed, not added to history. Only the structured report
            is saved locally in this browser.
          </small>
        </div>
        <span>3 AI models ready</span>
      </div>
      ...
      <div className="input-grid analyzer-grid">
        ...
        >
          {mode === "record" && (
            <div className={`capture-zone ${recording ? "recording" : ""}`}>
              ...
              <h2>
                {recording
                  ...
                  : "Record suspicious audio"}
              </h2>
              <p>
                {recording
                  ? "Keep the call audible. Recording stops automatically at 3:00."
                  : "Use speakerphone or play a saved voice note near your microphone."}
              </p>
              <button
                className={`button ${recording ? "danger-button" : "primary"}`}
                onClick={() =>
          ...
          {mode === "upload" && (
            <div
              className={`capture-zone ${dragging ? "dragging" : ""}`}
              onDragOver={(event) => {
              ...
              onDrop={(event) => {
                ...
                if (chosen)
                  acceptAudio(chosen).catch((caught) =>
                    setError(caught.message),
                  );
              }}
            >
              <div className="capture-icon">↑</div>
              <h2>Drop the recording here</h2>
              <p>WebM, OGG, MP4, M4A, WAV or MP3 · 25 MB · 3 minutes</p>
              <label className="button secondary" htmlFor="audio-upload">
                Choose audio file
              </label>
              <input
                id="audio-upload"
                hidden
                type="file"
                accept="audio/*,.webm,.ogg,.mp4,.m4a,.wav,.mp3"
                onChange={(event) => {
          ...
          {mode === "text" && (
            <div className="text-entry">
              <label htmlFor="evidence-text">Message or call transcript</label>
              <textarea
                id="evidence-text"
                value={text}
                onChange={(event) => {
              ...
              <div className="sample-chips">
                <span>Try a verified sample:</span>
                {TEXT_SAMPLES.map((sample) => (
                  ...
                  >
                    {FAMILY_META[sample.family].name}
                  </button>
                ))}
              </div>
            </div>
          ...
          {mode === "screenshot" && (
            <div className="capture-zone">
              <div className="capture-icon">▣</div>
              <h2>Scan a chat screenshot</h2>
              <p>
                English and Hindi OCR runs on this device before extracted text
                is sent for analysis.
              </p>
              <label className="button secondary" htmlFor="image-upload">
                Choose screenshot
              </label>
              <input
                id="image-upload"
                ...
                onChange={(event) => {
                  const chosen = event.target.files?.[0];
                  if (chosen) readScreenshot(chosen);
                }}
              />
              {ocrProgress !== null && (
                <div className="ocr-progress">
                  <i style={{ width: `${ocrProgress}%` }} />
                  <span>Reading image {ocrProgress}%</span>
                </div>
              )}
              {text && (
                <textarea
          ...
          {previewUrl && (
            <div className="audio-preview">
              ...
              <div>
                ...
                <button aria-label="Remove audio" onClick={() => setFile(null)}>
                  ×
                </button>
              </div>
            </div>
          ...
          <div className="pipeline" aria-live="polite">
            ...
            <span className={step > 2 ? "done" : step === 2 ? "active" : ""}>
              <i>2</i>
              <b>Classify</b>
              <small>Family + stages</small>
            </span>
          ...
          <button
            className="button primary analyze-submit"
          ...
          >
            {busy ? (
              ...
              <>
                Run three-model analysis <span>→</span>
              </>
            )}
          </button>
        </motion.section>
        ...
        <aside className="analyzer-aside">
          <section className="panel">
            <div className="panel-title">
              <div>
                ...
                <h3>One-click audio demos</h3>
              </div>
            </div>
            <p className="muted">
              Reenactments are clearly synthetic. Real samples are captured
              robocalls from the NCSU dataset.
            </p>
            <div className="demo-list">
              {demoManifest.map((demo) => (
                <button
                  ...
                  disabled={busy}
                  key={demo.id}
                  onClick={() => runDemo(demo.filename)}
                >
                  <span
                    className={`demo-kind ${demo.kind === "real_recording" ? "real" : ""}`}
                  >
                    {demo.kind === "reenactment" ? "R" : "REAL"}
                  </span>
                  <div>
                    <strong>
                      {demo.family
                        ? FAMILY_META[demo.family as keyof typeof FAMILY_META]
                            .name
                        : "Captured robocall"}
                    </strong>
                    <small>
                      {demo.kind === "reenactment"
                        ? "Synthetic reenactment"
                        : "Research dataset audio"}{" "}
          ...
          <section className="panel safety-note">
            <span>!</span>
            <div>
              <b>If money has moved</b>
              <p>
                Stop analysis and call 1930 first. Notify the bank through its
                official number.
              </p>
